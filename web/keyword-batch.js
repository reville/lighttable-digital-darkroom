...
export function installKeywordBatch(ctx) {
  ...
  function sync() {
    ...
    el('keywordBatchScope').textContent = count ? `${count} selected photos` : 'Select photos in a grid to tag them together.';
  }
  async function run(action) {
    if (busy) return;
    const names = [...ctx.names()];
    const values = ctx.values();
    if (action !== 'undo' && (!names.length || !values.length)) return toast('Select photos and enter keywords first');
    busy = true; sync();
    el('keywordInput').disabled = true; el('keywordAdd').disabled = true;
    try {
      if (!await ctx.flush()) throw new Error('Save pending edits before changing keywords');
      const result = await post('/api/catalog/keywords', {action, names, keywords: values, undoId});
      if (!result.ok || result.error) throw new Error(result.error || 'Keywords could not be saved');
      ctx.apply(result.changes || []);
      undoId = result.undoId || '';
      el('keywordInput').value = '';
      toast(`${result.count} ${result.count === 1 ? 'photo' : 'photos'} ${action === 'undo' ? 'restored' : 'updated'}`);
