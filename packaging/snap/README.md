...
x86_64 bundle for immutable release **0.5.0**, source
`be537f2f3e2e431ae6b42af716c2a8b365f57bab`. Its native notices and runtime
adjustment are pinned to that release and reject other versions or revisions.
The generator only prepares the package; Store registration and publication are separate steps.
...
python3 scripts/linux/make-snap-package.py dist/LightTable-0.5.0-linux-x86_64.tar.gz \
  --version 0.5.0 --source-revision be537f2f3e2e431ae6b42af716c2a8b365f57bab --output-dir .build/snap
cd .build/snap
snapcraft pack
```
...
it preserves existing output directories. The manual **Snap release candidate**
workflow downloads and validates that exact public release without creating a
GitHub release. **Linux sandbox candidates** delegates its Snap job to the same
pinned workflow; its version input applies to its other package jobs.
