...
ROUTE_COVERAGE = {
    ...
    "catalog": ["/api/storage", "/api/catalog", "/api/catalog/backup",
        ...
        "$defs": {
            ...
            "exportRecipe": {
                ...
                "properties": {
                    ...
                    "format": {"enum": ["jpeg", "heif", "png", "tif"], "default": "jpeg"},
                    "quality": {"type": "integer", "minimum": 1, "maximum": 100, "default": 92},
                    "sizeMode": {"enum": ["full", "long-edge", "fit", "short-edge", "megapixels", "percent"],
                                 "description": "Resize rule; omitted means long-edge when longEdge is set, otherwise full."},
                    "longEdge": {"type": ["integer", "null"], "minimum": 320, "maximum": 40000},
                    "maxWidth": {"type": ["integer", "null"], "minimum": 1, "maximum": 40000},
                    "maxHeight": {"type": ["integer", "null"], "minimum": 1, "maximum": 40000},
                    "shortEdge": {"type": ["integer", "null"], "minimum": 1, "maximum": 40000},
                    "megapixels": {"type": ["number", "null"], "minimum": 0.01, "maximum": 1600},
                    "percent": {"type": ["number", "null"], "minimum": 1, "maximum": 400},
                    "noEnlarge": {"type": "boolean", "default": True,
                                  "description": "Never upsize; a resize rule larger than the photo leaves it at full size."},
                    "resolutionPpi": {"type": ["integer", "null"], "minimum": 1, "maximum": 10000,
                                      "description": "Resolution tag only; pixel dimensions are unchanged."},
                    "sharpen": {"type": "object", "properties": {
                        "target": {"enum": ["none", "screen", "matte", "glossy"], "default": "none"},
                        "amount": {"enum": ["low", "standard", "high"], "default": "standard"}},
                        "description": "Output sharpening applied after resizing; print targets scale with resolutionPpi."},
                    "maxFileKb": {"type": ["integer", "null"], "minimum": 20,
                                  "description": "JPEG only: lower quality (never below 40) until the file fits, at most six encodes."},
                    "bitDepth": {"enum": [8, 16], "default": 16, "description": "TIFF sample depth; PNG and HEIF keep 8-bit output."},
                },
            },
            "stateUpdate": {
