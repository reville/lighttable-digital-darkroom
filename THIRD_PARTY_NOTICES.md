...
- Hair selection uses Google MediaPipe Selfie Multiclass Segmentation and the LiteRT runtime (Apache-2.0). `scripts/fetch-hair-model.py` verifies the pinned model hash; release bundles include `HairSegmentation-APACHE-2.0.txt` and model provenance in `hair-model.json`. LiteRT retains its distribution license and notices. The identical pinned asset also drives cross-platform subject and person selection (`film_lab_ai/subject_segmentation.py`), unioning the model's hair/body-skin/face-skin/clothes channels into one person silhouette; no additional model, license file, or fetch step is introduced for that reuse.
