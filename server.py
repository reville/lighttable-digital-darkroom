...
def array_shared_input(rgb: np.ndarray, cache_key: str):
    ...
    try:
        if sys.platform.startswith("linux"):
            ...
            if reserve is None:
                raise OSError("Linux shared-memory reservation is unavailable")
            ...
            for attempt in range(2):
                ...
                try:
                    ...
                    if not isinstance(result, dict) or not isinstance(result.get("ok"), bool):
                        raise ValueError("invalid resident Rust engine response")
                ...
                if self.cpu_fallback:
                    result.setdefault("fallback_reason", "Render worker stopped responding; using CPU for this session")
    ...
    try:
        ...
        if RUST_WORKER_BIN and cp["profile_enabled"]:
            ...
            detail = (f"{metrics.get('backend', 'cache')} "
                      f"{metrics.get('total_ms', 0):.0f} ms render")
        else:
