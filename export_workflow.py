...
def clean_sizing(raw: dict, *, long_edge_floor: int = MIN_EDGE) -> dict:
    """Validate the resize block; an old ``longEdge`` recipe keeps its meaning.

    ``long_edge_floor`` only raises the UI-facing 320 px minimum when cleaning
    a saved recipe (see ``clean_recipe``); the geometry itself, and internal
    callers like ``render_cli``/``resize_target`` that already trust their
    input, accept any positive edge.
    """
    raw = raw if isinstance(raw, dict) else {}
    long_edge = _optional_int(raw.get("longEdge"), long_edge_floor, MAX_EDGE)
...
def clean_recipe(raw: dict | None, *, builtin: bool = False) -> dict:
    ...
    try:
        ...
        quality = 92
    sizing = clean_sizing(raw, long_edge_floor=320)
    try:
...
def output_dimensions(width: int, height: int, *, rotate=0, crop=None,
                      long_edge=None, sizing: dict | None = None,
                      border=None) -> tuple[int, int]:
    """Mirror export's rotation, pixel-rounded crop, resize, then border."""
    ...
    width, height = resize_target(width, height, sizing)
