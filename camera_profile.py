...
"""DNG camera-profile (``.dcp``) reading and application.
...
``tifffile`` and applies the profile's tables in the order the DNG
specification (chapter 6) prescribes, on the linear ProPhoto RGB that the
capture stage produces:

1. ``ProfileHueSatMapData1``/``2``, blended at the capture white-balance
   temperature with the specification's 1/T weighting, in the encoding
   ``ProfileHueSatMapEncoding`` names (linear, or sRGB-encoded lookups).
2. Exposure: the Develop stage's own exposure normalisation, then the
   profile's ``BaselineExposureOffset`` as a linear gain.
3. ``ProfileLookTableData`` in its ``ProfileLookTableEncoding``.
4. ``ProfileToneCurve``, applied as the hue-preserving RGB tone the
   specification describes rather than three independent channel curves.

The colour matrices (``ColorMatrix1``/``2``, ``ForwardMatrix1``/``2``) are
applied at the RAW decode by ``camera_calibration``, which also estimates the
capture temperature this module blends the tables at. ``DefaultBlackRender``
is honoured trivially: LightTable applies no automatic black subtraction, so
``None`` (1) and ``Auto`` (0) render alike; the tag is reported so the
interface can say so. ``ProfileEmbedPolicy`` is read and reported; nothing
here embeds a profile anywhere.

What remains approximate is named in ``camera_calibration``'s docstring and
in ``docs/camera-profiles.md``: the demosaic and highlight reconstruction
come from LibRaw rather than a DNG reference converter, automatic white
balance estimates its temperature from the as-shot neutral, and the
calibration illuminants are taken at their nominal temperatures. Each
``profile_summary`` lists, in ``unsupported``, anything a specific file
carries that is not applied.

The grid interpolation is the standard trilinear one over (hue, saturation,
value) with a wrapping hue axis.

One profile is bundled: ``LightTable Standard``, a modelled generic look
generated by ``profiles/build_lighttable_standard.py`` from stated numeric
targets. It carries no camera matrices, so the decoder's own colour stays in
charge and only the look applies. Everything else comes from the folder the
user points at; no network access happens anywhere in this module.

Tag numbers follow the DNG specification (cross-checked against
``tifffile.TIFF.TAGS``): 50708 is ``UniqueCameraModel``, 50937 is
``ProfileHueSatMapDims``, 50941 is ``ProfileEmbedPolicy``, 50932 is
``ProfileCalibrationSignature``, 50981 is ``ProfileLookTableDims`` and 50982
is ``ProfileLookTableData``.
"""
...
# The bundled generic look, addressed by the same bare file name an edit
# stores for any other profile. ``bundled_profile_path`` resolves it without
# a user folder.
BUNDLED_PROFILE_DIR = Path(__file__).resolve().parent / "profiles"
BUNDLED_STANDARD_NAME = "LightTable Standard.dcp"
BUNDLED_STANDARD_FILE = BUNDLED_PROFILE_DIR / "lighttable_standard.dcp"
BUNDLED_STANDARD_LABEL = "LightTable Standard"

# DNG 1.4 ProfileHueSatMapEncoding / ProfileLookTableEncoding values.
ENCODING_LINEAR = 0
ENCODING_SRGB = 1

# The temperature the dual-illuminant tables blend at when the capture
# illuminant is unknown, matching camera_calibration.DEFAULT_CAPTURE_TEMPERATURE.
DEFAULT_TABLE_TEMPERATURE = 5500.0
...
APPLICATION_NOTE = (
    "Applied per the DNG specification: colour and forward matrices blended at "
    "the capture white balance replace the decoder's matrix; the hue/saturation "
    "map, baseline exposure offset, look table and tone curve follow in that "
    "order. Demosaic and highlight reconstruction come from LibRaw."
)
# Kept under its old name for callers that report it.
APPROXIMATION_NOTE = APPLICATION_NOTE
...
def _parse_page(page, file_path: Path) -> dict:
    ...
    model_tag = tags.get(TAG_UNIQUE_CAMERA_MODEL)
    ...
    profile = {
        ...
        "cameraModel": _tag_text(model_tag) if model_tag is not None else None,
        "copyright": (
    ...
    profile["approximate"] = bool(profile["unsupported"])
    profile["notes"] = APPLICATION_NOTE
    return profile
...
def _has_profile_content(profile: dict) -> bool:
    keys = (
        "name", "cameraModel", "copyright", "calibrationSignature",
        "hueSatDims", "hueSatMap1", "lookDims", "lookTable", "toneCurve",
        "forwardMatrix1", "colorMatrix1", "illuminant1", "embedPolicy",
    )
    return any(profile.get(key) is not None for key in keys)


def _unsupported_for(profile: dict) -> list[str]:
    """Name what this file carries that the pipeline does not apply.

    The matrices, both illuminants, the tables and their encodings, the
    baseline exposure offset and the tone curve are all applied, so a
    well-formed profile lists nothing here. What remains is a forward or
    colour matrix for a second illuminant without the first (the
    specification forbids it and the blend cannot use it) and an encoding
    value outside the two the specification defines.
    """
    skipped: list[str] = []
    if (profile.get("colorMatrix2") is not None
            and profile.get("colorMatrix1") is None):
        skipped.append("ColorMatrix2 without ColorMatrix1")
    if (profile.get("forwardMatrix2") is not None
            and profile.get("forwardMatrix1") is None):
        skipped.append("ForwardMatrix2 without ForwardMatrix1")
    if (profile.get("hueSatMap2") is not None
            and profile.get("illuminant2") is None):
        skipped.append(
            "ProfileHueSatMapData2 without CalibrationIlluminant2 (only "
            "table 1 is used)")
    for key, label in (("hueSatMapEncoding", "ProfileHueSatMapEncoding"),
                       ("lookTableEncoding", "ProfileLookTableEncoding")):
        value = profile.get(key)
        if value is not None and value not in (ENCODING_LINEAR, ENCODING_SRGB):
            skipped.append(f"{label} value {value} (treated as linear)")
    return skipped
...
def profile_summary(path) -> dict:
    ...
    return {
        ...
        "hueSatMapEncoding": profile["hueSatMapEncoding"],
        "lookTableEncoding": profile["lookTableEncoding"],
        "baselineExposureOffset": profile["baselineExposureOffset"],
        "defaultBlackRender": profile["defaultBlackRender"],
        "bundled": file_path == BUNDLED_STANDARD_FILE,
...
def bundled_profile_path(name) -> Path | None:
    """The bundled file a bare profile name addresses, or None.

    Only ``LightTable Standard.dcp`` is bundled. The comparison ignores
    case, as the user's own profile folder does on the platforms that ship
    one.
    """
    clean = " ".join(str(name or "").split()).strip()
    if not clean or clean.casefold() != BUNDLED_STANDARD_NAME.casefold():
        return None
    return BUNDLED_STANDARD_FILE if BUNDLED_STANDARD_FILE.is_file() else None


def is_bundled_name(name) -> bool:
    clean = " ".join(str(name or "").split()).strip()
    return bool(clean) and clean.casefold() == BUNDLED_STANDARD_NAME.casefold()


# --- Identity without tifffile ----------------------------------------------

_IDENTITY_TAGS = (TAG_UNIQUE_CAMERA_MODEL, TAG_PROFILE_NAME)
_IDENTITY_MAX_BYTES = 4096


def profile_identity(path) -> dict:
    """``{"name", "cameraModel"}`` from a profile's header, reading almost nothing.

    A profile folder can hold thousands of files, and ``read_profile`` loads
    every table in each. Matching a camera to its profiles needs only two
    short text tags, so this walks the first IFD by hand and reads just
    those payloads. Any structural problem yields ``{"name": None,
    "cameraModel": None}`` rather than an exception; the full reader reports
    real errors when a file is actually chosen.
    """
    empty = {"name": None, "cameraModel": None}
    try:
        with open(path, "rb") as handle:
            ...
            magic, offset = struct.unpack(f"{order}HI", header[2:8])
            if magic != 42 or offset < 8:
                return empty
            handle.seek(offset)
            count_bytes = handle.read(2)
            if len(count_bytes) < 2:
                return empty
            (count,) = struct.unpack(f"{order}H", count_bytes)
            directory = handle.read(12 * count)
            if len(directory) < 12 * count:
                return empty
            found: dict[int, str | None] = {}
            for index in range(count):
                entry = directory[12 * index:12 * index + 12]
                code, kind, length, payload = struct.unpack(f"{order}HHI4s", entry)
                if code not in _IDENTITY_TAGS or kind not in (1, 2, 7):
                    continue
                if length > _IDENTITY_MAX_BYTES:
                    continue
                if length <= 4:
                    raw = payload[:length]
                else:
                    (position,) = struct.unpack(f"{order}I", payload)
                    handle.seek(position)
                    raw = handle.read(length)
                    if len(raw) < length:
                        continue
                text = raw.decode("utf-8", "replace").replace("\x00", "").strip()
                found[code] = text or None
    except (OSError, struct.error, ValueError):
        return empty
    return {"name": found.get(TAG_PROFILE_NAME),
            "cameraModel": found.get(TAG_UNIQUE_CAMERA_MODEL)}
...
def _grid_encoding(encoding) -> int:
    """Normalise a DNG table encoding; anything undefined reads as linear."""
    try:
        value = int(encoding) if encoding is not None else ENCODING_LINEAR
    except (TypeError, ValueError):
        return ENCODING_LINEAR
    return ENCODING_SRGB if value == ENCODING_SRGB else ENCODING_LINEAR


def _apply_hsv_grid(rgb, table, dims, label: str,
                    encoding=ENCODING_LINEAR) -> np.ndarray:
    ...
    ``encoding`` is the DNG 1.4 table encoding: with ``ENCODING_SRGB`` the
    lookup runs on sRGB-encoded values and the result is decoded again, as
    the specification requires for tables authored in that domain.
    """
    ...
    encoded = _grid_encoding(encoding) == ENCODING_SRGB
    if encoded:
        clipped = _srgb_encode(clipped).astype(work, copy=False)
    ...
    result = _hsv_to_rgb(hue, saturation, value)
    if encoded:
        result = _srgb_decode(result)
    return result.astype(work, copy=False)


def apply_hue_sat_map(rgb, table, dims, encoding=ENCODING_LINEAR) -> np.ndarray:
    ...
    ``encoding`` is the profile's ``ProfileHueSatMapEncoding``.
    """
    return _apply_hsv_grid(rgb, table, dims, "ProfileHueSatMapData", encoding)


def apply_look_table(rgb, table, dims, encoding=ENCODING_LINEAR) -> np.ndarray:
    """Apply a ProfileLookTable to RGB in 0..1.

    Identical grid maths to ``apply_hue_sat_map``; the difference in DNG is
    where in the pipeline it lands: after exposure, before the tone curve.
    ``encoding`` is the profile's ``ProfileLookTableEncoding``.
    """
    return _apply_hsv_grid(rgb, table, dims, "ProfileLookTableData", encoding)


def apply_baseline_exposure_offset(rgb, offset) -> np.ndarray:
    """Apply ``BaselineExposureOffset`` (in EV) as a linear gain, clipped."""
    source, work = _as_rgb(rgb)
    try:
        stops = float(offset or 0.0)
    except (TypeError, ValueError):
        stops = 0.0
    if not np.isfinite(stops) or stops == 0.0:
        return source
    gain = np.asarray(2.0 ** stops, dtype=work)
    return np.clip(source * gain, 0.0, 1.0).astype(work, copy=False)
...
def apply_profile(rgb, profile, *, strength: float = 1.0,
                  ...
                  expose=None) -> np.ndarray:
    """Apply a profile's tables in DNG order to linear ProPhoto RGB in 0..1.

    ``profile`` is a dict from ``read_profile`` (a path is accepted too and is
    read on the spot). The stages, in order: the hue/sat map blended at
    ``temperature`` (the capture white balance in kelvin; both tables and
    both illuminants must be present to blend, and a missing temperature
    blends at :data:`DEFAULT_TABLE_TEMPERATURE`), then ``expose`` — the
    caller's own exposure step, applied between the map and the look as the
    specification places it — then ``BaselineExposureOffset``, the look
    table and the tone curve. Each table honours its encoding tag.

    ``strength`` blends linearly between the input and the fully applied
    result and is clamped to 0..1, so ``strength=0`` returns the input
    unchanged; ``expose`` still runs on the input in that case so the
    caller's exposure is never lost.

    The colour matrices are not applied here; they belong to the decode
    ...
    exposed_source = source if expose is None else np.asarray(
        expose(source), dtype=work)
    if amount == 0.0:
        return exposed_source.copy()
    ...
    if (hue_sat_map is not None and hue_sat_map_2 is not None
        ...
        blend_at = (DEFAULT_TABLE_TEMPERATURE if temperature is None
                    else float(temperature))
        hue_sat_map = interpolate_hue_sat_maps(
            hue_sat_map, hue_sat_map_2, illuminant1, illuminant2, blend_at)

    if hue_sat_map is not None and hue_sat_dims is not None:
        result = apply_hue_sat_map(
            result, hue_sat_map, hue_sat_dims,
            profile.get("hueSatMapEncoding"))

    if expose is not None:
        result = np.asarray(expose(result), dtype=work)

    result = apply_baseline_exposure_offset(
        result, profile.get("baselineExposureOffset"))

    look_table = profile.get("lookTable")
    look_dims = profile.get("lookDims")
    if look_table is not None and look_dims is not None:
        result = apply_look_table(
            result, look_table, look_dims, profile.get("lookTableEncoding"))
    ...
    if amount >= 1.0:
        return result
    return (exposed_source + (result - exposed_source) * amount).astype(
        work, copy=False)
